...
import org.asmeta.interpreter.UpdateClashException;
import org.asmeta.interpreter.UpdateSet;
import org.asmeta.interpreter.main.AsmModelNotFoundException;
...
public abstract class RunJob extends Job {
	...
	public final IStatus run(IProgressMonitor monitor) {
		// GET THE CONSOLE
		IOConsole mc = org.asmeta.eclipse.Utility.findDefaultConsole();
		mc.activate();
		...
		try {
			...
			if (outputfromSim == null) {
				...
				//Logger.getLogger("org.asmeta.interpreter").setAdditivity(false);
				Logger.getLogger("org.asmeta.interpreter").addAppender(outputfromSim);
			}
	...
	final LocationSet runASM(IProgressMonitor monitor, PrintStream printOut,
		...
		while (!checkToStop(monitor, us)) {
			...
			try {
				savePreviousState(asmSimulator);

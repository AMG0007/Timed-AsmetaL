...
public class OCL_Checker {
	...
	public static boolean isLong(Object t) {
	...
	public static boolean I7_I8_I9(FunctionInitialization f_init) {
		...
		// I7
		if (size == 1)
			if (((VariableTerm) f_init.getVariable().get(0)).getDomain() != D) {
...
*/
	...
	public static boolean B7_B8_B9(FunctionDefinition f_def) {
		...
		// B7
		if (size == 1)
			if (((VariableTerm) f_def.getVariable().get(0)).getDomain() != D) {
				MSG_ERR = "Error: The domain where the formal parameter takes its value"
						+ "must be equal to the domain of the function.";
				return false;
			} else
...
//			return -1;
	...
	public static boolean R1(UpdateRule r) {
		...
		if (!(compatible(r.getLocation(), r.getUpdatingTerm()))) {
			MSG_ERR = "Error: In an update rule, the term on the right-hand side must be compatible "
	...
	// --- Delayed RULE --- //
		public static boolean checkDelayedRule(DelayedRule r) {
			return R20(r);
		}

		public static boolean R20(DelayedRule r) {
			// Check constraint R3: guard.domain.oclIsTypeOf(BooleanDomain)
			if (!isLong(r.getGuard()) || r.getGuard() < 0) {
				MSG_ERR = "Error: In a delayed rule, the delay is a Long.";
				return false;
			}
			return true;
		}
	...
	public static boolean E2(TupleTerm t) {
		// check constraint E2
		if (t.getArity() == 1
				&& t.getDomain() != ((Term) t.getTerms().get(0)).getDomain()) {

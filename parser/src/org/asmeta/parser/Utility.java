...
public class Utility {
	//PA 12/11/2011. Fabio Albani, in method resolve, needs to import just the the first rule with
	...
	// ================================================= appendInKey
	/*
	 * Concatenate to key the representation of t depending on the attribute
	 * symbol key = "" -> t = 10 : returns "10" key "[3," -> t = {3,4} returns
	 * "[3,{3,4}"
	 */
		...
		if (t instanceof TupleTerm) {
			...
			while (iter.hasNext()) {
				...
				if (tupleElem instanceof FunctionTerm) {
					key.append(((FunctionTerm) tupleElem).getFunction()
							.getName());
	...
	// ================================================= getCommonTD
	/*
	 * Se td ha TD fissato controllo che la funz compare(td, elem_TD) torni true
	 * se td ha TD generico (non ho ancora incotrato un termine con TD fissato)
	 * e l'elem corrente ha TD generico -> passo al prossimo elemento (non
	 * faccio alcun controllo) se td ha TD generico (non ho ancora incotrato un
	 * termine con TD fissato) e l'elem corrente ha TD fissato -> assegno a td
	 * tale dominio e passo al prossimo elem Se non vengono riscontrate
	 * differenze tra i type-domain degli elementi della collection viene
	 * tornato il TypeDomain comune altrimenti viene tornato null NOTA: se tutti
	 * gli elementi della collection hanno come type-domain AnyDomain (sono
	 * variabili) il TD del collectionTerm sar? uno structuredTD su AnyDomain
	 */
	public static TypeDomain getCommonTD(Collection<Term> c) {
		...
		while (iter.hasNext()) {
			elem_TD = OCL_Checker.getTypeDomain(iter.next()
					.getDomain());
	...
	private static RealTerm/*Term*/ convertDoubleToTerm(double n, TermsFactory termsPack,
			TypeDomain realDom, HashMap<String, List<Function>> declared_Func,
			HashMap<String, Domain> declared_Dom, DefinitionsFactory defPack, Signature s)
			throws ParseException {
		...
		// otherwise create a FunctionTerm for -n
		TupleTerm tupleT = createSingle(realTerm, termsPack, defPack, s);
		FunctionTerm t = createFunctionTerm("minus", tupleT, termsPack,
				declared_Func, declared_Dom, defPack, s);
		if (t == null)
			throw new ParseException(
					"Problems in creating the unary expression for" + n);
		return t;*/
	}
 ...
 * @throws ParseException
 */
	private static IntegerTerm/*Term*/ convertIntegerToTerm(int n, TermsFactory termsPack,
			TypeDomain integerDom,
			HashMap<String, List<Function>> declared_Func,
			HashMap<String, Domain> declared_Dom, DefinitionsFactory defPack, Signature s)
			throws ParseException {
		...
		// otherwise create a FunctionTerm -n
		assert( n < 0);
		integerTerm.setSymbol(Integer.toString(n));
		TupleTerm tupleT = createSingle(integerTerm, termsPack, defPack, s);
		FunctionTerm t = createFunctionTerm("minus", tupleT, termsPack, declared_Func, declared_Dom, defPack, s);
		if (t == null)
			throw new ParseException(
					"Problems in creating the unary expression for" + n);
		return t;*/
	...
	// ================================================= convertToDouble
	/**
	 * Returns the double for a FunctionTerm (Unary Expression with domain Real
	 * or Integer) or for a constant numeric term (real, integer, natural).
	 * 
	 * @throws ParseException
	 */
	...
	public static double convertToDouble(Term ft) throws ParseException {
		...
		if (!(ft instanceof FunctionTerm))
			throw new ParseException(
					"Can convert only function terms like + constant or - constant, found  "
							+ ft.getClass());
		// get the unary Expression
		String name = ((FunctionTerm) ft).getFunction().getName();
		if (!(((FunctionTerm) ft).getFunction().getArity() == 1 && (name
				.equals("plus") || name.equals("minus"))))
			throw new ParseException(
					"Can convert only unary expressions + constant or - constant ");

		Term argument = (Term) ((FunctionTerm) ft).getArguments().getTerms()
				.get(0);

		if (!(argument instanceof RealTerm)
				&& !(argument instanceof IntegerTerm))
			throw new ParseException(
	 ...
	 * remaining elements till lastElem, taking into account of the step and of
	 * the type. By default the step is 1. The interval notation is allowed only
	 * for reals, integer and natural numbers!
	 * 
	 * @throws ParseException
	 */
	public static void createTermCollection(Term firstElem, Term lastElem,
			Collection<Term> elemColl, double step, boolean natural_step,
			TermsFactory termsPack,
			HashMap<String, List<Function>> declared_Func,
			HashMap<String, Domain> declared_Dom, DefinitionsFactory defPack, Signature s)
			throws ParseException {
		...
		if (low > upp)
			throw new ParseException(
					"Error: The last interval element cannot be greatest than the first one.");
		...
		// Set of natural terms
		{
			...
			TypeDomain naturalDom = getBasicDomain(defPack.getDomains(),
					"Natural");
			if (naturalDom == null) {
				throw new ParseException(
						"Error: The natural domain has not been declared.");
			}
			// put the elements
			logger.debug("\t\t\tInterval elements: "
					+ ((NaturalTerm) firstElem).getSymbol());
		...
		else if (natural_step
				&& (firstElem.getDomain() instanceof IntegerDomain))
		// Set of integer terms
		{
			...
			// HashMap
			TypeDomain integerDom = getBasicDomain(defPack.getDomains(),
					"Integer");
			if (integerDom == null) {
				throw new ParseException(
						"Error: The integer domain has not been declared.");
			}
			...
			// stored
			{
				...
				Term intToadd = convertIntegerToTerm(i, termsPack, integerDom,
						declared_Func, declared_Dom, defPack, s);
		...
		if (firstElem.getDomain() instanceof RealDomain) {
			if (!(lastElem.getDomain() instanceof RealDomain))
				throw new ParseException(
						"Error: The first interval element is real so also the last one must be real."
								+ "(The TypeDomain associated to every element of a set must be the same!)");
			...
			// HashMap
			TypeDomain realDom = (TypeDomain) getBasicDomain(defPack
					.getDomains(), "Real");
			if (realDom == null) {
				throw new ParseException(
						"Error: The real domain has not been declared.");
			}
			...
			// already stored
			{
				...
				elemColl.add(convertDoubleToTerm(i, termsPack, realDom,
						declared_Func, declared_Dom, defPack, s));
				// add a new association
	...
	public static Domain fixTypeDomain(Domain td, HashMap<String, Domain> m,
			HashMap<String, Domain> declared_Dom, DefinitionsFactory defPack, Signature s) {
		// ProductDomain
		if (td instanceof ProductDomain) {
			...
			while (iter.hasNext())
				newlist.add((TypeDomain) fixTypeDomain(iter.next(), m,
						declared_Dom, defPack, s));
			return getProduct(newlist, defPack, s);
		}
		// PowersetDomain
		else if (td instanceof PowersetDomain)
			return getPowerset(fixTypeDomain(((PowersetDomain) td)
					.getBaseDomain(), m, declared_Dom, defPack, s), defPack, s);
		// SequenceDomain
		else if (td instanceof SequenceDomain)
			return getSequence(fixTypeDomain(((SequenceDomain) td).getDomain(),
					m, declared_Dom, defPack, s), defPack, s);
		// BagDomain
		else if (td instanceof BagDomain)
			return getBag(fixTypeDomain(((BagDomain) td).getDomain(), m,
					declared_Dom, defPack, s), defPack, s);
		// MapDomain
		else if (td instanceof MapDomain)
			return getMap(fixTypeDomain(((MapDomain) td).getSourceDomain(), m,
					declared_Dom, defPack, s), fixTypeDomain(((MapDomain) td)
					.getTargetDomain(), m, declared_Dom, defPack, s), defPack, s);
			...
			try {
				...
				if (dom == null) {
					...
					dom = (Domain) getPredefinedDomain(
							defPack.getDomains(), dom_name);
				}
				if (dom == null) {
					logger.debug("Trying structured domain");
					dom = (Domain) getStructuredDomain(
							defPack.getDomains(), dom_name, s);
				}
				if (dom == null)
					throw new ParseException("Error: " + dom_name
							+ " domain is not declared.");
			} catch (Exception e) {
				logger.error("Problems in fixing the type domain "
						+ td.getName());
	...
	public static RuleDomain getRuleDomain(List<Domain> domainList,
		...
		RuleDomain domReturned = (RuleDomain) getStructuredDomain(defPack
				.getDomains(), name, s);
		if (domReturned == null) // it is not present in the HashMap
		{ // create the object
			logger.debug("RuleDomain: " + name
					+ " not present in the HashMap. It is now been created..");
			logger.debug("domainList: " + name + " of size: "
					+ domainList.size());
			RuleDomain ruleDom;
	 ...
	 */
	public static ProductDomain getProduct(List<Domain> domainList,
			DefinitionsFactory defPack, Signature s) {
		String name = getDomName(domainList);

		// check if this structuredTD has been defined previously
		ProductDomain domReturned = (ProductDomain) getStructuredDomain(defPack
				.getDomains(), name, s);
		if (domReturned == null) // it is not present in the HashMap
		{ // create the object
	...
	public static PowersetDomain getPowerset(Domain dom,
			DefinitionsFactory defPack, Signature s) {
		String name = "Powerset(";
		name = name.concat(dom.getName());
		name = name.concat(")");
		// check if this structuredTD has been defined previously
		PowersetDomain powersetDom = (PowersetDomain) getStructuredDomain(
				defPack.getDomains(), name, s);
	...
	// OK!
	public static SequenceDomain getSequence(Domain dom,
			DefinitionsFactory defPack, Signature s) {
		String name = "Seq(";
		name = name.concat(dom.getName());
		name = name.concat(")");
		// check if this structuredTD has been defined previously
		SequenceDomain seqDom = (SequenceDomain) getStructuredDomain(defPack
				.getDomains(), name, s);
	...
	public static BagDomain getBag(Domain dom, DefinitionsFactory defPack, Signature s) {
		...
		name = name.concat(")");
		// check if this structuredTD has been defined previously
		BagDomain bagDom = (BagDomain) getStructuredDomain(
				defPack.getDomains(), name, s);
	...
	public static MapDomain getMap(Domain dom1, Domain dom2,
			DefinitionsFactory defPack, Signature s) {
		String name = "Map(";
		...
		name = name.concat(")");
		// check if this structuredTD has been defined previously
		MapDomain mapDom = (MapDomain) getStructuredDomain(
				defPack.getDomains(), name, s);
		...
		{ // create the object
			...
			mapDom.setName(name);
			// set references
			mapDom.setSourceDomain((TypeDomain) dom1);
			mapDom.setTargetDomain((TypeDomain) dom2);
			//set the signature
	 ...
	 * "Rule", se la lista ha un solo elemento D restituisce D altrimenti
	 * restitiuce Rule(D1,...DN). A rule domain can be formed also by concrete
	 * domains.
	 */
	...
	/**
	 * Per generare il nome di un product se la lista e' vuota restituisce "",
	 * se la lista ha un solo elemento D restituisce D altrimenti restitiuce
	 * Prod(D1,...DN). A product domain can now be formed also by concrete
	 * domains.
	 */
	public static <T extends Domain> String getDomName(List<T> domains) {
		...
		while (iter.hasNext()) {
			d = iter.next();
			/*
			 * if (size > 1 && (d instanceof ConcreteDomain)) return null; //
			 * THIS NO LONGER APPLY: A product domain cannot be formed by
			 * concrete domains else
			 */
			dom_Name = dom_Name.concat(d.getName()).concat(",");
		}
		dom_Name = dom_Name.substring(0, dom_Name.length() - 1); // delete
	...
	/**
	 * It creates a function term if the funcname function is dynamic, it
	 * returns a LocationTerm otherwise it returns a FunctionTerm
	 * 
	 * @param funcName
	 *            functionName: it cannot be null
	 * @param tupleT
	 *            the tuple argument of the function: it can be null
	 */
	public static FunctionTerm createFunctionTerm(String funcName,
			TupleTerm tupleT, TermsFactory termsPack,
			HashMap<String, List<Function>> declared_Func,
			HashMap<String, Domain> declared_Dom, DefinitionsFactory defPack, Signature s)

	{
		...
		try {
			...
			logger.debug("\t\t\tFound function "
					+ func.getName()
					+ ((func.getDomain() != null) ? " with domain "
							+ func.getDomain().getName() : ""));
		} catch (Exception e) {
			logger.error("Problems locating the function " + funcName
					+ ((tupleT != null) ? ("(" + tupleD.getName() + ")") : ""));
			logger.error("Exception: " + e.getMessage());
			logger.debug("tupleD has domain " + (tupleD != null ? tupleD.getName() : "") 
					+ " and ref " + (tupleD != null ? tupleD.toString() : ""));
			// get the functions with the same name
			List<Function> f_list = declared_Func.get(funcName);
			if (f_list == null)
				logger.debug("No other funtions with the same name!");
			else {
				logger
						.debug("other functions with the same name have domains: ");
				for (Function f : f_list)
					logger.debug(f.getDomain().getName() + " ref: "
							+ f.getDomain().toString());
			}
			return null;
		}
		// create the object
	 ...
	 * @param withCompare
	 *            the with compare
	 * @param genericDomValue
	 *            the generic dom value
	 * @param declared_Func
	 *            the declared_ func
	 * 
	 * @return the function
	 */
	public static Function getFunction(String f_name, Domain d,
			boolean withCompare, HashMap<String, Domain> genericDomValue,
			HashMap<String, List<Function>> declared_Func) {
		...
		if (f_list == null)
			...
			while (iter.hasPrevious()) {
				...
				if (d == null && f_dom == null)
					...
					if (withCompare) {
						...
						if (OCL_Checker.compareFixingAnyDomain(f_dom, d,
								genericDomValue)) {
							logger.debug("\t\t getFunction " + f_name
									+ " Domain " + d.getName() + " found ");
							return func;
						}
			...
			logger.debug("\t\t getFunction " + f_name + " Domain "
					+ (d != null ? "d.getName()" : " null") + " not found ");
			return null;
		}
	}
	...
	public static RuleDeclaration search_ruleName(
			Collection<RuleDeclaration> collection, String obj_name) {
		return searchNameInCollection(collection, obj_name);
	}

	public static Function search_funcName(Collection<Function> collection,
			String obj_name) {
		return searchNameInCollection(collection, obj_name);
	}
	...
	private static <T extends asmeta.structure.NamedElement> T searchNameInCollection(
			Collection<T> collection, String obj_name) {
	 ...
	 */
	public static boolean insert(Function f,
			HashMap<String, List<Function>> declared_Func) {
	...
	public static boolean remove(Function f,
			HashMap<String, List<Function>> declared_Func) {
		String f_name = f.getName();
		List<Function> f_list = declared_Func.get(f_name);
		if (f_list == null)
			...
			for (int count = 0; count < f_list.size(); count++) {
				...
				if (OCL_Checker.getTypeDomain(func.getDomain()) == OCL_Checker
						.getTypeDomain(d)) {
					f_list.remove(count);
					return true;
				}
	...
	public static boolean insert(Domain d, HashMap<String, Domain> declared_Dom) {
		...
		if (declared_Dom.get(d_name) == null) {
			if (!(d instanceof StructuredTd || d instanceof BasicTd
					|| d instanceof AgentDomain || d instanceof ReserveDomain || d instanceof AnyDomain))
	...
	/**
	 * binds a variable to a term: the term must have domain a PowersetDomain or
	 * a SequenceDomain like in SetT { $o | $o in X} -> X is a term the variable
	 * get the domain the base domain of the term
	 * 
	 * TODO TO BE COMPLETED WITH OTHER KIND OF DOMAIN ATTENZIONE lega la
	 * variabile al TypeDomain e non al domain nel caso di un concrete domain
	 * 
	 * @throws ParseException
	 */
	public static void updateVariable(VariableTerm v, Term inTerm)
			throws ParseException {
		...
		if (itD instanceof PowersetDomain)
			...
			throw new ParseException("binding variable " + v.getName() + " to "
					+ inTerm.getDomain().getName() + " not allowed");
		}
		v.setDomain(D);
	...
	public static StringTerm createStringT(String s, TermsFactory termsPack,
			DomainsFactory Dom) {
		// look for the String domain reference in the predefined_Dom HashMap
		TypeDomain dom = (TypeDomain) getBasicDomain(Dom, "String");
		if (dom == null)
			return null;
	...
	public static TupleTerm createSingle(Term arg1, TermsFactory termsPack,
			DefinitionsFactory defPack, Signature s) {
	 ...
	 * association
	 * 
	 * if the term is unique, the domain of the tuple is equal to the domain of
	 * the term
	 */
	public static TupleTerm createPair(Term arg1, Term arg2,
			TermsFactory termsPack, DefinitionsFactory defPack, Signature s) {
	...
	/**
	 * given a Domain Factory, returns a basic domain with name name if name is
	 * not valid, return null
	 * 
	 * @throws ParseException
	 *             in case the basic domain is not known
	 * 
	 */
	...
	/**
	 * get the AbstarctDomains agent and reserve if they exists, otherwise
	 * create them
	 */
	...
	/**
	 * Given a name, returns the ruleDeclaration with that name, null if there
	 * are problems.
	 * 
	 * @param declared_Rules all the declaration rules
	 * @param idElement
	 * @param actualParamList
	 *            the list of domains of idElement: if it is null, return the
	 *            only one in pratica posso fare l'import o ecport solo di una
	 *            regola ... e idem per gli invarianti
	 * @throws ParseException
	 * 
	 */
	static RuleDeclaration getRuleByNameTerm(
			Map<String, List<RuleDeclaration>> declared_Rules,
			String idElement, List<Term> actualParamList) throws ParseException {
	 ...
	 * 
	 * Given a list of terms, returns a list with the domains of the terms in
	 * the same order.
	 * 
	 * @param actualParamList
	 *            a list of terms
	...
	static <T extends Term> List<Domain> buildDomains(List<T> actualParamList) {
	 ...
	 /* Given a list of terms, returns a list with the domains of the terms in
	 * the same order. For concrete domains, the astract super domain is considered.
	 * 
	 * @param actualParamList
	 *            a list of terms
	 * @return the list of the term domains
	 */
	/*static <T extends Term> List<Domain> buildSuperDomains(List<T> actualParamList) {
		List<Domain> domains = new ArrayList<Domain>();
		for (Term term : actualParamList) {
			Domain dom = term.getDomain();
			if (dom instanceof ConcreteDomain) {
				ConcreteDomain cd = (ConcreteDomain)dom; 
			    domains.add(cd.getTypeDomain());
			   }
...
//		sb.deleteCharAt(sb.length() - 1);
	 ...
	 * @throws ParseException
	 */
	static RuleDeclaration getRuleByNameDom(
			Map<String, List<RuleDeclaration>> declared_Rules,
			String idElement, List<Domain> expDoms) throws ParseException {
	 ...
	 * org.asmeta.interpreter.StaticFunctionEvaluator.java
	 * 
	 * Given a macro name and the domains of the actual parameters,
	 * returns the macro declaration that best matches with the given one.
	 * 
	 * @param declared_Rules 
	 * 		rules declared in the model
	 * @param name 
	 * 		macro name
	 * @param expDoms 
	 * 		domains of the actual parameters
	 * @return 
	 * 		a macro declaration, null if none is found
	 * @throws ParseException 
	 * 		if there are too macros compatible with the given declaration
	 * 
	 */
	static RuleDeclaration resolve(
			Map<String, List<RuleDeclaration>> declared_Rules,
			String name, 
			List<Domain> expDoms) throws ParseException {
		StringBuilder sb = new StringBuilder();
		for(Domain domain: expDoms) sb.append(domain.getName() + " ");
		...
		for (int i = 0; i < rulesByName.size(); i++) {
			...
			for(Domain domain: currDoms) sb.append(domain.getName());
		...
		if (bestIndex == -1) {
			// no candidate found
			return null;
		} /* FIXME 25/06/2010 by Fabio Albani
		commentato per supportare import multipli di una stessa regola
		per poi selezionare solo la prima  
		
		else if (bestIndex2 != -1) {
			// too many canditates found with the same rating
			throw new ParseException("ERROR: Unresolved reference " + name
					+ " Too many compatible definitions: "
					+ rulesByName.get(bestIndex) + ", "
	 ...
	 * 
	 * @param declared_Rules
	 *            all the rules declared in the module
	 * @param idElement
	 *            a macro name
	 * @param actualParamList
	 *            a list of domains
	 ...
	 *         otherwise null
	 */
	static <T extends Term> RuleDeclaration searchRuleByName(
			Map<String, List<RuleDeclaration>> declared_Rules,
			String idElement, List<T> actualParamList) {
		/*
		 * commented 8 Jan 2008 by acarioni try { return
		 * getRuleByNameTerm(declared_Rules, idElement, actualParamList); }
	...
	/** read an import file
	 * it can be relative to the asmDirLib or absolute
	 * 
	 * @param asmDirLib can be null, in case "." is taken
	 * @param moduleName 
	 * @param asmExtension 
	 */
	static File importFile(String asmDirLib, String moduleName, String asmExtension){
		// it should compute again asmLibPath relatively the import ...??
		if (asmDirLib == null) asmDirLib = ".";
		// 	build the complete module name
		String importFileName = new String( asmDirLib + File.separatorChar + moduleName + asmExtension);
		File moduleFile = new java.io.File(importFileName);
		// if the relative does not work, try the absolute
		if (!moduleFile.exists()){
			//
			importFileName = new String(moduleName + asmExtension);
			moduleFile = new java.io.File(importFileName);
			if (!moduleFile.exists()){

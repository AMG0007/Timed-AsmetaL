...
public class StandardLibrary {
	...
	public static BooleanValue iff(BooleanValue op1, BooleanValue op2) {
		if (!op1.getValue() && !op2.getValue() || op1.getValue()
				&& op2.getValue())
			return BooleanValue.TRUE;
		else
			return BooleanValue.FALSE;
	...
	public static SequenceValue tail(SequenceValue seq1) {
		...
		List<Value> nl = new ArrayList<Value>(l.subList(1, seq1.getValue()
				.size()));
	...
	public static SequenceValue subSequence(SequenceValue sv, IntegerValue iv1,
		...
		List<Value> nl = new ArrayList<Value>(l.subList((int) iv1.getValue(),
				(int) iv2.getValue()));
		return new SequenceValue(nl);
	}
	...
	public static BooleanValue notallin(SetValue set1, SetValue set2) {
		return BooleanValue.parseBooleanValue(!set1.getValue().containsAll(
				set2.getValue()));
	}
	...
	public static IntegerValue indexOf(SequenceValue sq, Value v) {
		int result = -1;
		int index = 0;
		for (Value value: sq) {
			if (value.equals(v)) {
	...
	public static IntegerValue sum(SetValue sv) {
		long somma = 0;
	...
	public static IntegerValue sum(BagValue sv) {
		long somma = 0;
		for(Value v: sv.getValue()) {
			somma = somma + ((IntegerValue)v).getValue();
		}
	...
	public static RuleValue program(AgentValue agent) {
		...
		if (program == null) {
			throw new RuntimeException("Agent " + agent.getId()
					+ " has not program");
		}
	...
	public static Value third(TupleValue t) {
		return t.getValue()[2];
	}
	
	public static Value fourth(TupleValue t) {
		return t.getValue()[3];
	}

	public static Value fifth(TupleValue t) {
		return t.getValue()[4];
	}

	public static Value sixth(TupleValue t) {
		return t.getValue()[5];
	}

	public static Value seventh(TupleValue t) {
		return t.getValue()[6];
	}

	public static Value eighth(TupleValue t) {
		return t.getValue()[7];
	}

	public static Value nineth(TupleValue t) {
		return t.getValue()[8];
	}
	...
	public static IntegerValue print(Value v1, Value v2, Value v3, Value v4,
			Value v5, Value v6, Value v7, Value v8, Value v9, Value v10) {

...
/**
 * PA (31/10/2010)
 * It discovers if a term contains monitored functions.
 * It is used in the invariant verification to discover if an invariant contains
 * monitored functions.
 * Indeed an invariant, depending on the fact that it contains or does not contain
 * monitored functions, must be evaluated in different points of the execution.
 *
 */
public class MonitoredFinder extends ReflectiveVisitor<Boolean> {
	...
	public boolean visit(FunctionTerm funcTerm) {
		return Defs.isMonitored(funcTerm.getFunction()) ||
				visit(funcTerm.getArguments());
	}
